...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    /*
    const celebrated = "";
    enIyilerListesi.forEach((developer) => {
      if (programciId === developer.id) {
        celebrated === developer.isim;
      }
    });
    return celebrated;*/
    return programcilar[programciId - 1].isim;
  };
      ...
      <div className="programmers">
        {
          ...
          programcilar.map((dev) => (
            <div className="programmer" key={dev.id}>
              ...
              <button
                onClick={() => {
                  /* burada dev.id 'yi öne çıkan id'ye atayın */ setProgramciId(
                    dev.id
                  );
                }}
              >
